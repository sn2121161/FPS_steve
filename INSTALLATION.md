...
2. Download and extract tarball:
    ...
    ```
    git clone git@github.com:Flexible-Power-Systems/SteVe.git
    cd steve
    ```
...
4. Configure SteVe to Access Azure Key Vault:
   - Login to Azure from console with following command
   ```
    az login
   ```
   ...
   - Use the above returned credentials information to set AZURE_CLIENT_ID (appId), AZURE_CLIENT_SECRET (password), and AZURE_TENANT_ID (tenant) environment variables. The following example shows a way to do this in Bash:
     Run following bash command on where the SteVe will be executed.
   ```
